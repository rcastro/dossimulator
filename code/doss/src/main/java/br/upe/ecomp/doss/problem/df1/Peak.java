...
public class Peak {
	...
	public void changePeak(int iteration) {
		if (dfParameters.isDynamicH()) {
			hi = logisticFunction.generateDynamicH(hi, dfParameters.getHBase(),
					dfParameters.getHRange(), dfParameters.getHScale(), iteration,
					dfParameters.getAH());
			// System.out.println("Hi: " + hi);
		}
		if (dfParameters.isDynamicR()) {
			ri = logisticFunction.generateDynamicR(ri, dfParameters.getRBase(),
					dfParameters.getRRange(), dfParameters.getRScale(), iteration,
					dfParameters.getAR());
		}
		...
		if (dfParameters.isDynamicX()) {
			xi = logisticFunction.generateDynamicX(xi, dfParameters.getXBase(),
					dfParameters.getXRange(), dfParameters.getXScale(), iteration,
					dfParameters.getAX());
